export default function DocumentsB() {
  ...
  return (
    <section id="documents" className="py-20 sm:py-32 bg-[#111111]">
        ...
        <div className="mt-12 grid sm:grid-cols-2 gap-4 max-w-3xl mx-auto">
          {documents.map((doc, index) => (
            <a
              key={index}
              href={doc.href}
              className="flex items-start gap-4 p-6 bg-white/5 rounded-xl border border-white/10 hover:border-[#00B2A9]/50 transition-colors group"
            >
              <div className="w-12 h-12 bg-[#00B2A9]/10 text-[#00B2A9] rounded-lg flex items-center justify-center flex-shrink-0 group-hover:bg-[#00B2A9]/20 transition-colors">
                {doc.icon}
              </div>
              <div>
                <h3 className="font-semibold text-white group-hover:text-[#00B2A9] transition-colors">
                  {doc.title}
                </h3>
                <p className="mt-1 text-sm text-gray-500">
                  {doc.description}
                </p>
                <span className="inline-flex items-center gap-1 mt-2 text-sm font-medium text-[#00B2A9]">
                  Ladda ner PDF
